...
export class CanvasDocumentRenderer {
  ...
  renderLayer(layer, document, renderedLayerIds = []) {
    ...
    if (layer.mask?.enabled) {
      if (layer.mask.assetId) {
        ...
        if (maskAsset) canvas = this.applyAlphaSource(canvas, maskAsset, layer.mask.inverted, layer.mask.opacity);
      }
      const eraseStrokes = layer.mask.metadata?.eraseStrokes ?? [];
      if (eraseStrokes.length) canvas = this.applyEraseStrokes(canvas, eraseStrokes, document);
    }
    return canvas;
  }
  ...
  drawLayerContent(context, layer, document) {
    ...
    if (layer.type === 'paint') {
      this.drawPaintContent(context, layer, document);
      return;
    }
  ...
  drawPaintContent(context, layer, document) {
    const width = layer.content.width || document.width;
    const height = layer.content.height || document.height;
    const fills = layer.content.fills ?? [];
    const strokes = layer.content.strokes ?? [];
    const byId = new Map([...fills, ...strokes].map(operation => [operation.id, operation]));
    const orderedIds = (layer.content.operationOrder ?? []).filter(id => byId.has(id));
    for (const operation of [...fills, ...strokes]) if (!orderedIds.includes(operation.id)) orderedIds.push(operation.id);
    let cache = this.paintCache.get(layer.id);
    const reusable = cache
      && cache.width === width
      && cache.height === height
      && cache.operationIds.every((id, index) => orderedIds[index] === id)
      && cache.operationIds.length <= orderedIds.length;
    if (!reusable) {
      cache = { canvas: this.createCanvas(width, height), width, height, operationIds: [] };
      cache.canvas.getContext('2d')?.clearRect?.(0, 0, width, height);
    }
    const cacheContext = cache.canvas.getContext('2d');
    for (let index = cache.operationIds.length; index < orderedIds.length; index += 1) {
      const operation = byId.get(orderedIds[index]);
      if (operation?.points) this.drawPaintStroke(cacheContext, operation, width, height);
      else if (operation) this.drawPaintFill(cacheContext, operation, width, height);
      cache.operationIds.push(orderedIds[index]);
    }
    this.paintCache.set(layer.id, cache);
    context.drawImage(cache.canvas, 0, 0);
  }

  drawPaintFill(context, fill, width, height) {
    const operation = this.createCanvas(width, height);
    const operationContext = operation.getContext('2d');
    operationContext.save?.();
    operationContext.globalAlpha = Math.min(1, Math.max(0, Number(fill.opacity) || 0));
    if (fill.type === 'radial') {
      const radius = Math.max(1, Math.hypot(fill.end.x - fill.start.x, fill.end.y - fill.start.y));
      const gradient = operationContext.createRadialGradient?.(fill.start.x, fill.start.y, 0, fill.start.x, fill.start.y, radius);
      if (gradient) {
        gradient.addColorStop(0, fill.colorA);
        gradient.addColorStop(1, fill.colorB);
        operationContext.fillStyle = gradient;
      } else operationContext.fillStyle = fill.colorA;
    } else if (fill.type === 'linear') {
      const gradient = operationContext.createLinearGradient?.(fill.start.x, fill.start.y, fill.end.x, fill.end.y);
      if (gradient) {
        gradient.addColorStop(0, fill.colorA);
        gradient.addColorStop(1, fill.colorB);
        operationContext.fillStyle = gradient;
      } else operationContext.fillStyle = fill.colorA;
    } else operationContext.fillStyle = fill.color;
    operationContext.fillRect?.(0, 0, width, height);
    operationContext.restore?.();
    context.drawImage(this.applySelection(operation, fill.selection, width, height), 0, 0);
  }

  drawPaintStroke(context, stroke, width, height) {
    const operation = this.createCanvas(width, height);
    drawBrushStroke(operation.getContext('2d'), stroke);
    context.drawImage(this.applySelection(operation, stroke.selection, width, height), 0, 0);
  }

  applySelection(source, selection, width, height) {
    if (!selection?.entries?.length) return source;
    const mask = rasterizeSelection(selection, width, height);
    const maskCanvas = this.createCanvas(width, height);
    const context = maskCanvas.getContext('2d');
    if (!context?.createImageData || !context?.putImageData) return source;
    ...
    return this.applyAlphaSource(source, maskCanvas, false, 1);
  }

  applyEraseStrokes(source, strokes, document) {
    const output = this.createCanvas(source.width, source.height);
    const context = output.getContext('2d');
    context.drawImage(source, 0, 0);
    for (const stroke of strokes) {
      const operation = this.createCanvas(document.width, document.height);
      drawBrushStroke(operation.getContext('2d'), { ...stroke, color: '#000000' });
      const selected = this.applySelection(operation, stroke.selection, document.width, document.height);
      context.save?.();
...
function drawShape(context, content) {
  context.save?.();
  if (content.shadowBlur > 0 || content.shadowOffsetX || content.shadowOffsetY) {
    context.shadowColor = content.shadowColor || 'rgba(0,0,0,.35)';
    context.shadowBlur = content.shadowBlur || 0;
    context.shadowOffsetX = content.shadowOffsetX || 0;
    context.shadowOffsetY = content.shadowOffsetY || 0;
  }
  context.beginPath?.();
  if (content.shape === 'ellipse') {
    context.ellipse?.(content.width / 2, content.height / 2, content.width / 2, content.height / 2, 0, 0, Math.PI * 2);
  } else if (content.shape === 'line' || content.shape === 'arrow') {
    context.moveTo?.(0, 0);
    context.lineTo?.(content.width, content.height);
    if (content.shape === 'arrow') {
      const angle = Math.atan2(content.height, content.width);
      const size = Math.max(8, content.strokeWidth * 4);
      context.moveTo?.(content.width, content.height);
      context.lineTo?.(content.width - Math.cos(angle - Math.PI / 6) * size, content.height - Math.sin(angle - Math.PI / 6) * size);
      context.moveTo?.(content.width, content.height);
      context.lineTo?.(content.width - Math.cos(angle + Math.PI / 6) * size, content.height - Math.sin(angle + Math.PI / 6) * size);
    }
...
function drawBrushStroke(context, stroke) {
  if (!context || !stroke?.points?.length) return;
  const size = Math.max(1, Number(stroke.size) || 1);
  const dabs = resampleStroke(stroke.points, Math.max(0.25, size * Math.max(0.02, Number(stroke.spacing) || 0.2)));
  context.save?.();
  context.globalAlpha = Math.min(1, Math.max(0, Number(stroke.opacity) || 0));
  for (const point of dabs) {
    const radius = Math.max(0.5, size * (Number(point.pressure) || 1) / 2);
    context.beginPath?.();
    if (context.createRadialGradient && Number(stroke.hardness) < 0.999) {
      const hardness = Math.min(1, Math.max(0, Number(stroke.hardness) || 0));
      ...
      gradient.addColorStop(Math.max(hardness, 0.001), stroke.color);
      gradient.addColorStop(1, transparentColor(stroke.color));
      context.fillStyle = gradient;
    } else context.fillStyle = stroke.color;
...
function transparentColor(color) {
  ...
  if (/^#[0-9a-f]{3}$/i.test(color)) return `${color[0]}${color[1]}${color[1]}${color[2]}${color[2]}${color[3]}${color[3]}00`;
  return 'rgba(0,0,0,0)';
}
