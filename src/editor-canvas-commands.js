...
export function createResizeDocumentCommand(widthInput, heightInput, options = {}) {
  ...
  return createDocumentCommand('Zmień rozmiar dokumentu', documentModel => {
    ...
    const nextSelection = resizeSelection(documentModel.metadata?.selection, previous.width, previous.height, width, height);
...
function cropSelection(selectionInput, sourceWidth, sourceHeight, crop) {
  if (!selectionInput) return null;
  if (Math.abs(crop.angle) > 1e-6) return null;
  const source = rasterizeSelection(createSelection(selectionInput), sourceWidth, sourceHeight);
  const output = new Uint8ClampedArray(crop.width * crop.height);
  for (let y = 0; y < crop.height; y += 1) {
    for (let x = 0; x < crop.width; x += 1) {
      const sourceX = crop.x + x;
      const sourceY = crop.y + y;
      if (sourceX >= 0 && sourceY >= 0 && sourceX < sourceWidth && sourceY < sourceHeight) {
        output[y * crop.width + x] = source[sourceY * sourceWidth + sourceX];
      }
    }
  }
  if (!output.some(value => value > 0)) return null;
  return createSelection({
    width: crop.width,
    height: crop.height,
    entries: [{ operation: 'replace', shape: rasterSelection(output, crop.width, crop.height) }]
  });
}

function resizeSelection(selectionInput, sourceWidth, sourceHeight, width, height) {
  if (!selectionInput) return null;
  const source = rasterizeSelection(createSelection(selectionInput), sourceWidth, sourceHeight);
  const scalable = createSelection({
    width: sourceWidth,
    height: sourceHeight,
    entries: [{ operation: 'replace', shape: rasterSelection(source, sourceWidth, sourceHeight) }]
  });
  const output = rasterizeSelection(scalable, width, height);
  if (!output.some(value => value > 0)) return null;
  return createSelection({
    width,
    height,
    entries: [{ operation: 'replace', shape: rasterSelection(output, width, height) }]
  });
}
